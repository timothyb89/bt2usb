...
pub fn parse_report_map(data: &[u8]) -> Option<heapless::Vec<MouseReportLayout, MAX_LAYOUTS>> {
    ...
    while pos < data.len() {
        ...
        if pos + 1 + byte_count > data.len() {
            warn!("Report Map truncated at offset {}", pos);
            break;
        }
    ...
    for (i, cand) in candidates.iter().enumerate() {
        ...
        debug!(
            ...
            cand.report_id, total_bytes, cand.mouse_field_count
        );
        if let Some(ref f) = cand.buttons {
            debug!("  buttons: offset={} size={}", f.bit_offset, f.bit_size);
        }
        if let Some(ref f) = cand.x {
            debug!(
                "  X: offset={} size={} signed={}",
                f.bit_offset, f.bit_size, f.is_signed
            );
        }
    ...
    if result.is_empty() {
        debug!("Report Map: no mouse-like fields found");
        return None;
    }

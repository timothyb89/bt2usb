...
enum Command {
    ...
    /// Dump the embedded firmware ELF to a file (for diagnostics)
    #[command(name = "dump-elf")]
    DumpElf {
        /// Output file path
        #[arg(default_value = "embedded_firmware.elf")]
        output: String,
    },
}
...
fn cmd_dump_elf(output: &str) -> Result<()> {
    ...
    println!("Embedded ELF size: {} bytes", FIRMWARE_ELF.len());
    let magic = &FIRMWARE_ELF[..FIRMWARE_ELF.len().min(16)];
    println!("First 16 bytes:   {:02x?}", magic);
    let is_elf = FIRMWARE_ELF.len() >= 4 && &FIRMWARE_ELF[..4] == b"\x7fELF";
    println!(
        "ELF magic:        {}",
        if is_elf {
            "valid".green().to_string()
        } else {
            "INVALID (expected 7f 45 4c 46)".red().to_string()
        }
    );

    std::fs::write(output, FIRMWARE_ELF)?;
    println!("Dumped to: {output}");
    Ok(())
}
